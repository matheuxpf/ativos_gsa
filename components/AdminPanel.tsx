...
export const AdminPanel: React.FC<AdminPanelProps> = (props) => {
  ...
  const startEdit = (item: any) => {
    ...
    // Se estivermos editando funcionários, precisamos encontrar o roleId correspondente ao texto 'role'
    if (activeTab === 'EMPLOYEES') {
      const matchingRole = props.roles.find(r => r.description === item.role && r.region === item.region);
      if (matchingRole) {
        editState.roleId = matchingRole.id; // Isso faz o <select> marcar a vaga correta
      }
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (activeTab === 'EMPLOYEES') {
      ...
      const payload = {
        ...dadosLimpos,
        role: selectedRole ? selectedRole.description : formState.role // Mantém o anterior se não mudar
      };
      ...
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
          ...
          <div className="p-6 max-h-[60vh] overflow-y-auto">
            {userAssets.length === 0 ? (
              <div className="text-center py-12 text-slate-400 flex flex-col items-center">
                ...
                <p>Nenhum ativo sob responsabilidade deste funcionário.</p>
              </div>
            ) : (
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-50 text-xs uppercase font-bold text-slate-500">
                  <tr><th className="p-3 rounded-l-lg">Identificação</th><th className="p-3">Tipo/Marca</th><th className="p-3 text-right rounded-r-lg">Valor</th></tr>
                </thead>
          ...
          <div className="bg-slate-50 p-4 border-t border-slate-200 flex justify-between items-center">
             <span className="text-xs font-bold text-slate-400 uppercase">Total sob custódia</span>
        ...
        {data.map((item: any) => (
          <tr key={item.id} className="text-sm hover:bg-slate-50 transition-colors">
              ...
              {activeTab === 'ASSETS' ? (
                <div>
                 ...
                 <div className="flex flex-col gap-1">
                    ...
                    <span className={`text-[10px] uppercase font-black px-2 py-0.5 rounded w-fit border ${
                      item.status === 'Ativo' ? 'bg-green-100 text-green-700 border-green-200' :
                      item.status === 'Desligado' ? 'bg-red-100 text-red-700 border-red-200' :
            ...
            <td className="px-6 py-4 text-right whitespace-nowrap">
              {activeTab === 'EMPLOYEES' && (
                <button onClick={() => setViewingAssets(item)} title="Ver Ativos" className="text-slate-500 hover:bg-slate-100 hover:text-gsa-blue p-2 rounded mr-2 transition-colors">
                  <Package size={16}/>
                </button>
              )}
              {/* USANDO A NOVA FUNÇÃO STARTEDIT AQUI */}
